				...
				switch ( $current_page_type ) {
					...
					case 'is_author':
						...
						if ( 'is_tax' === $current_page_type && ( is_category() || is_tag() || is_tax() ) ) {
							...
							if ( is_object( $q_obj ) ) {
								$meta_args .= " OR pm.meta_value LIKE '%\"" . $current_post_type . '|all|taxarchive|' . esc_sql( $q_obj->taxonomy ) . "\"%'";
								$meta_args .= " OR pm.meta_value LIKE '%\"tax-" . esc_sql( $q_obj->term_id ) . "\"%'";
							}
					...
					case 'is_singular':
						...
						if ( $current_post_obj && $current_post_obj->post_parent ) {
							...
							if ( ! empty( $parent_ids ) ) {
								foreach ( $parent_ids as $pid ) {
									$meta_args .= " OR (pm.meta_value LIKE '%\"specifics\"%' AND pm.meta_value LIKE '%\"apply_on_child\";b:1%' AND pm.meta_value LIKE '%\"post-" . esc_sql( $pid ) . "\"%')";
								}
							}
						}
						...
						if ( is_object( $q_obj ) ) {
							...
							foreach ( $terms as $term ) {
								$meta_args .= " OR pm.meta_value LIKE '%\"tax-" . esc_sql( $term->term_id ) . '-single-' . esc_sql( $term->taxonomy ) . "\"%'";
							}
						}
