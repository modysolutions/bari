...
//		add_filter( 'plugin_row_meta', array( $this, 'plugin_row_meta' ), 10, 2 );
	...
	function menu() {
		...
		add_submenu_page(
            'tamarind-base',
            __( 'Import users', 'import-users-from-csv-with-meta' ),
            __( 'Import users', 'import-users-from-csv-with-meta' ),
            apply_filters( 'acui_capability', 'create_users' ),
            'acui',
            array( $acui_import, 'show' ),
            999999
        );
	}
	
	function admin_enqueue_scripts( $hook ) {
		if( 'tamarind-base_page_acui' == $hook ){
            error_log($hook);
		...
		if( 'users.php' == $hook ){
			...
			wp_localize_script( 'acui_js', 'acui_js_object',
				array(
					'import_url' => get_admin_url( null, 'admin.php?page=acui&tab=homepage' ),
					...
					'export_url' => get_admin_url( null, 'admin.php?page=acui&tab=export' ),
	...
	function action_links( $links, $file ) {
		if ($file == 'import-users-from-csv-with-meta/import-users-from-csv-with-meta.php') {
			$links[] = sprintf( __( '<a href="%s">Export</a>', 'import-users-from-csv-with-meta' ), get_admin_url( null, 'admin.php?page=acui&tab=export' ) );
			$links[] = sprintf( __( '<a href="%s">Import</a>', 'import-users-from-csv-with-meta' ), get_admin_url( null, 'admin.php?page=acui&tab=homepage' ) );
